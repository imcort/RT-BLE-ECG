...
//#include "mpu6050.h"
//定义I2C引脚
//#define TWI_SCL_M       NRF_GPIO_PIN_MAP(0,22)     //SCL引脚
//#define TWI_SDA_M       NRF_GPIO_PIN_MAP(0,23)     //SDA引脚
...
uint8_t max30102_FIFO_ReadBytes(uint8_t *i2c_data)
{
	ret_code_t err_code;
	//uint32_t un_temp;
	...
	//读
	m_xfer_done = false;
	err_code = nrf_drv_twi_rx(&m_twi, MAX30102_ADDRESS, i2c_data, 6);
	//APP_ERROR_CHECK(err_code);
		return true;
}
//读MAX30102 FIFO
...
//init_flag=max30100_read(MAX30100_INT_STATUS);
